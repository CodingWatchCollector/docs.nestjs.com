...
> info **Hint** File presence is required by default, but you can make it optional using `fileIsRequired` parameter inside `build` function (at the same level as `errorHttpStatusCode`).
